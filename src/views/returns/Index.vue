<template>
  <div>
    <CRow>
      <CCol xs="12" lg="12">
        <CCard>
          <CCardHeader> Returns </CCardHeader>
          <CCardBody>
            <div>
              <CButton
                color="success"
...
<script>
import ReturnByProductService from "@/services/returns/ReturnByProductService.js";
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "return_cash", label: "Return Cash", _style: "width:20%" },
        {
          key: "status",
          label: "Status",
          _style: "width:20%;",
  ...
  methods: {
    ...
    getData(page = "", per_page = "") {
      this.getServerData(ReturnByProductService, page, per_page);
    },
    editReturn(k) {
    ...
    viewRow(uuid) {
      this.$router.push({ path: "/returns/show/" + uuid });
    },
    editRow(uuid) {
      // this.$router.push({ path: "/catalogs/brands/edit/" + uuid });
    },
    deleteRow(uuid) {
      this.deleteData(ReturnByProductService, uuid);
    },
  },
};
</script>
