...
export default {
  ...
  watch: {
    ...
    activePage() {
      this.getEmployeeAllowance(this.activePage, this.perPage);
    },
  },
  methods: {
    getEmployeeAllowance(page = "", per_page = "") {
      this.empId = this.$route.params.id;
      EmployeeAllowanceService.getAll(this.empId, page, per_page)
        .then(({ data }) => {
    ...
    onTableChange() {
      setTimeout(() => {
        ...
        this.employeeAllowanceData = agent.currentItems;
        this.pages = Math.ceil(agent.sortedItems.length / 5);
      }, 1000);
    },
    changePagination(value) {
      this.perPage = parseInt(value);
      this.getEmployeeAllowance("", this.perPage);
    },
  },
};
</script>
