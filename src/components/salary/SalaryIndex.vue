            ...
            <td>
              <CButtonGroup>
                <!-- <CButton
                    ...
                    > -->
                <CButton
                  v-if="$ability.can('update', 'employee-salary-adjustments')"
                ...
                /></CButton>
                <CButton
                  v-if="$ability.can('destroy', 'employee-salary-adjustments')"
...
const fields = [
  ...
  { key: "month", label: "MONTH", _style: "min-width:15%" },
...
export default {
  ...
  methods: {
    getEmployeeSalary(page = "", per_page = "") {
      EmployeeSalaryAdjustmentService.getAll("", page, per_page, true)
        .then(({ data }) => {
          console.log(data);
