...
export default {
  ...
  methods: {
    getReturns() {
      ...
      ProductExchangePaymentService.getAll(1, 100)
        .then(({ data }) => {
          // console.log(data);
    ...
    savePayment() {
      ...
      if (!this.$v.$invalid) {
        ...
        let data = this.form;
        ProductExchangePaymentService.store(data)
          .then((res) => {
    ...
    updatePayment() {
      ...
      if (!this.$v.$invalid) {
        this.$store.commit("set_loader");
        this.form.product_exchange_id = this.$route.params.id;
        let data = this.form;
        ProductExchangePaymentService.update(this.form.id, data)
          .then((res) => {
            if (res.status == 200) {
              this.$swal.fire({
                icon: "success",
